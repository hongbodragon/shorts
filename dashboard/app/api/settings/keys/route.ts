...
import { getApiKeys, addApiKey, toggleApiKey, deleteApiKey } from "@/lib/db";
...
export async function POST(req: NextRequest) {
  const { service_name, api_key, key_label } = await req.json();
  if (!service_name || !api_key) return NextResponse.json({ error: "missing fields" }, { status: 400 });
  addApiKey(service_name, api_key, key_label ?? "");
  return NextResponse.json({ ok: true });
}

export async function PATCH(req: NextRequest) {
  const { id, is_active } = await req.json();
  toggleApiKey(id, is_active);
  return NextResponse.json({ ok: true });
}
